...
[![Platform](https://img.shields.io/badge/platform-linux%20%7C%20macOS-lightgrey)](https://github.com/neginnilforosh/HMM_KunitzDomain)
...
│       └── contaminated_ids.txt           # 8 Swiss-Prot entries excluded from structural training (≥95% identity to PDB seeds)
│
├── Scripts/
...
# Remove contaminated sequences from the full positive set for use in
# structural training alignment only; all 211 sequences are kept for cross-validation
python3 -c "
...
print(f'Kept {len(records)} sequences after contamination removal (for structural training only)')
"

# Split all 211 positives into 2 folds for cross-validation
python3 -c "
from Bio import SeqIO
...
# Step 3b: extract domain subsequences (40–100 aa) from full Swiss-Prot sequences
python3 Scripts/extract_kunitz_domains.py
